...
public abstract partial class BaseLLMProvider : ILLMProvider
{
    protected readonly TornadoApi Api;
    protected readonly LLMProviderOptions Options;
    protected readonly ITokenEstimatorFactory TokenEstimatorFactory;
    public abstract string ProviderName { get; }
    public abstract BenchmarkProvider BenchmarkProviderType { get; }
    public abstract LLmProviders ProviderType { get; }

    protected BaseLLMProvider(LLMProviderOptions options, ITokenEstimatorFactory tokenEstimatorFactory, TornadoApi api)
    {
        Options = options;
    ...
    public async Task<List<LLMResponse>> ExecuteAsync(LLMRequest request, CancellationToken cancellationToken = default)
    {
        var results = new List<LLMResponse>();

        foreach (var modelConfig in Options.Models)
        {
            var currentModel = modelConfig.Model;
            var currentVendor = modelConfig.Vendor;
            ...
            try
            {
                ...
                var model = new ChatModel(modelConfig.Model, ProviderType);
            ...
            catch (Exception ex)
            {
                results.Add(new LLMResponse
                {
                    Provider = ProviderName + " -> " + currentVendor,
                    Model = currentModel,
                    Success = false,
                    Error = ex.ToString()
                });
